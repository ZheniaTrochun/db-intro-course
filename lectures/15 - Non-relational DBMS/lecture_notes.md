...
Векторні СУБД зберігають дані у вигляді числових векторів (embedding-ів) - масивів чисел, які описують семантичне значення об'єктів (текстів, зображень, аудіо тощо).
Основна операція - пошук найближчих сусідів (Nearest Neighbor Search): знайти вектори, які найбільш "схожі" на заданий вектор за певною метрикою відстані (косинусна подібність, евклідова відстань).

Як створюються вектори:
- Текст, зображення або інші дані пропускаються через ML-модель (наприклад, OpenAI Embeddings, BERT), яка перетворює їх у числовий вектор фіксованої розмірності (наприклад, 768 або 1536 чисел)
- Семантично схожі об'єкти мають близькі вектори - це дозволяє шукати не за точним збігом ключових слів, а за змістом

Переваги:
- Семантичний пошук - знаходить результати за змістом, а не за точним збігом слів (наприклад, запит "домашній улюбленець" знайде документи про котів та собак)
- Основа для RAG (Retrieval-Augmented Generation) - паттерн, при якому LLM отримує релевантний контекст з векторної бази перед генерацією відповіді
- Ефективний пошук серед мільйонів об'єктів завдяки спеціалізованим індексам (HNSW, IVF)

Недоліки:
- Якість пошуку повністю залежить від якості ML-моделі, яка генерує вектори
- Високе споживання пам'яті - кожен вектор займає значний об'єм (1536 float32 = ~6 КБ на запис)
- Оновлення індексів може бути повільним при великих об'ємах даних

Приклади СУБД: Pinecone, Weaviate, Qdrant.

Варто зазначити, що PostgreSQL (як і багато інших "класичних" СУБД) підтримує роботу з векторами через розширення `pgvector` - це дозволяє зберігати вектори та виконувати пошук найближчих сусідів без додаткової СУБД.
...
```mermaid
flowchart TD
      A[Потрібна БД для реалізації нового функціоналу] --> L{На проекті вже використовується якась СУБД?}
      L -->|Так| M{Можна її використати?}
      M -->|Так| N[Використовуємо вже існуючу СУБД]
      M -->|Ні| B{Можна її використати?}
      L -->|Ні| B{Потрібні зв'язки між таблицями?}
      B -->|Так| D[Реляційна СУБД - PostgreSQL]
      B -->|Ні| C{Потрібні гарантії ACID?}
      C -->|Так| D
      C -->|Ні| E{Який тип даних?}
      E -->|Ієрархічні / JSON| F[Документна - MongoDB]
      E -->|Кеш / сесії| G[Ключ-значення - Redis]
      E -->|Графові зв'язки| H[Графова - Neo4j]
      E -->|Повнотекстовий пошук| I[Пошуковий рушій - ElasticSearch]
      E -->|Аналітика на великих даних| J[Колонкова - Snowflake]
      E -->|Масивний запис + масштабування| K[Column-Family - Cassandra]
      E -->|Семантичний пошук / RAG| R[Векторна - Pinecone / pgvector]
```

---

## 7. Рекомендації щодо вибору СУБД для нового проекту
...
Підхід, коли в межах однієї системи використовується велика кількість різних СУБД називається "Polyglot Persistence" і виглядає приблизно наступним чином:

```mermaid
flowchart TD
      A[Застосунок] --> |основна база даних| B[PostgreSQL]
      A --> |кеш, сесії| C[Redis]
      A --> |логи| D[ElasticSearch]
      A --> |дані для аналітики| E[Snowflake]
      A --> |дані для RAG| F[Pinecone]
```
...
- Векторні СУБД (Pinecone, pgvector) - для семантичного пошуку та RAG-систем на основі ML-моделей
...
- [What is Snowflake](https://www.datacamp.com/blog/what-is-snowflake)
