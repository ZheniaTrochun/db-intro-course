...
-- Рішення:
WITH student_avg AS (
SELECT s.student_id, CONCAT(s.first_name, ' ', s.last_name) AS full_name, s.group_id, AVG(e.grade) AS avg_student_grade
FROM student s
JOIN enrolment e ON s.student_id = e.student_id
WHERE e.grade IS NOT NULL
GROUP BY s.student_id, s.first_name, s.last_name, s.group_id
),
group_avg AS (
...
SELECT sa.student_id, sa.full_name, g.name AS group_name,
...
JOIN student_group g ON sa.group_id = g.group_id
