...
JOIN campus.groups g ON s.group_id = g.id
JOIN (
    SELECT student_id, AVG(grade) as personal_avg 
    FROM campus.enrolments GROUP BY student_id
) avg_s ON s.id = avg_s.student_id
WHERE avg_s.personal_avg > (
    SELECT AVG(e2.grade)
    FROM campus.students s2
    JOIN campus.enrolments e2 ON s2.id = e2.student_id
