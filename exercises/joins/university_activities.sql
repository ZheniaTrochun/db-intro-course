...
(
    SELECT 
        CONCAT(p_stud.first_name, ' ', p_stud.last_name) AS full_name,
...
(
    SELECT 
        CONCAT(p_prof.first_name, ' ', p_prof.last_name) AS full_name,
        c.name AS course_name,
        'викладання курсу' AS activity_type
    FROM course_teacher ct
    JOIN professor pr ON ct.professor_id = pr.professor_id
    JOIN person p_prof ON pr.person_id = p_prof.person_id
