...
SELECT p.first_name || ' ' || p.last_name as student_name,
    ...
    e.grade as grade,
    prof_p.first_name || ' ' || prof_p.last_name as lecturer_name
  FROM enrolment as e
...
JOIN person prof_p ON pr.person_id = prof_p.person_id
 WHERE e.grade < 60 
   AND e.grade IS NOT NULL 
   AND ct.professor_role::text = convert_from('\xd0bbd0b5d0bad182d0bed180'::bytea, 'UTF8')
